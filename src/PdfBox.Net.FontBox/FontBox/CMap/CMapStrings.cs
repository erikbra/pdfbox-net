...
public static class CMapStrings
{
    ...
    public static string? GetMapping(byte[] bytes)
    {
        ...
        return bytes.Length == 1 ? OneByteMappings[CMap.ToInt(bytes)] : TwoByteMappings[CMap.ToInt(bytes)];
    }
    ...
    public static byte[]? GetByteValue(byte[] bytes)
    {
        ...
        return bytes.Length == 1 ? OneByteValues[CMap.ToInt(bytes)] : TwoByteValues[CMap.ToInt(bytes)];
    }
}
