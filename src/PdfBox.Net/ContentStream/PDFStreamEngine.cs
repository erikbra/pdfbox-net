...
/// <summary>
/// Stub for PDFStreamEngine base class.
/// Full port pending - this is the base for text extraction engines.
/// </summary>
public class PDFStreamEngine
{
    private readonly List<OperatorProcessor> _operators = new();
    private readonly PDGraphicsState _graphicsState = new();
    private Matrix _textMatrix = new();
    private PDPage? _currentPage;

    internal void AddOperator(OperatorProcessor op)
    {
        if (op != null)
        {
            _operators.Add(op);
    ...
    protected PDGraphicsState GetGraphicsState()
    {
        return _graphicsState;
    }

    protected Matrix GetTextMatrix()
    {
        return _textMatrix;
    }

    protected void SetTextMatrix(Matrix textMatrix)
    {
        _textMatrix = textMatrix ?? new Matrix();
    }

    protected PDPage? GetCurrentPage()
    {
        return _currentPage;
    }
