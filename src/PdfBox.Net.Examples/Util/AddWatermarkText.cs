...
/// <summary>
/// Add a watermark text to each page of a PDF document.
/// </summary>
public class AddWatermarkText
{
    ...
    public static void Main(string[] args)
    {
        if (args.Length != 3)
        {
            Console.Error.WriteLine("usage: AddWatermarkText <input-pdf> <output-pdf> <text>");
            return;
        }

        using (PDDocument doc = Loader.LoadPDF(args[0]))
        {
            // NOTE: PDPageContentStream text drawing operators (BeginText, SetFont, SetTextMatrix,
            // ShowText, EndText) and PDType0Font loading by font name are not yet implemented
            // in this .NET port.
