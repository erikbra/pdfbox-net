...
- Keep each region narrowly scoped and include a short reason comment so future sync reviews can validate intent.
- If local code intentionally changes behavior/API semantics, ensure file metadata is set to `PORT_MODE: adapted`.
