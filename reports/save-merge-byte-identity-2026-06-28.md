...
Source: GitHub Actions runtime-parity artifact from PR #550 run 28329163828
Runtime parity output: `artifacts/runtime-parity-issue-539-download/runtime-parity-28329163828-1`
...
| Area | Issue | Goal |
|---|---|---|
| Metadata/trailer IDs | #551 | Stabilize writer-generated metadata and trailer IDs where deterministic output is explicitly requested or used by parity tests. |
| COS dictionary serialization | #552 | Compare Java/.NET dictionary key sequence differences and implement low-risk writer-local stabilization if practical. |
| Streams and compression | #553 | Align writer-controlled filter/compression choices where platform deflater internals do not prevent stable output. |
| Object numbering and xref layout | #554 | Measure object allocation/xref layout causes and stabilize only where it preserves COS correctness and Java-source comparability. |
| Incremental-save behavior | #555 | Add focused parity fixtures for incremental-save markers, `/Prev` chains, appended xref sections, and byte preservation semantics. |
