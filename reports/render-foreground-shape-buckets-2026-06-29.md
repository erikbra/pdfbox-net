...
Source: GitHub Actions runtime-parity run 28374229554 reclassified for #562
Runtime parity output: `artifacts/runtime-parity-issue-562-download/runtime-parity-28374229554-1`
Runtime comparison generated UTC: `2026-06-29T13:29:19Z`
...
|---|---:|---:|---:|
| `render-foreground-shape-equivalence-match` | 47 | 38 | 38 |
| `render-rotation-overlay-shape-equivalence-match` | 0 | 9 | 9 |
...
| `render-foreground-shape-equivalence-match` | 38 | PageDrawer geometry, clipping, and backend rasterization | Accepted shape-equivalence bucket with real geometry/clipping slices split out. | #562 |
| `render-rotation-overlay-shape-equivalence-match` | 9 | page rotation, overlay placement, and page-boundary strokes | Reviewed rotation/overlay shape bucket; keep separate from generic foreground-shape rows. | #562 |
| `render-image-mask-shape-equivalence-match` | 2 | sampled images, stencil masks, and image masks | Real renderer gap; reduce fixture by fixture. | #559 |
| `render-jbig2-decoder-raster-equivalence-match` | 1 | JBIG2 decoder and bitonal image rasterization | Reviewed JBIG2 decoder raster bucket; keep separate from broader sampled-image and mask rows. | #559 |
| `render-pattern-transparency-raster-equivalence-match` | 1 | patterns, transparency groups, and form XObjects | Real renderer gap; reduce fixture by fixture. | #560 |
| `render-global-resource-text-raster-equivalence-match` | 4 | LiveCycle global-resource text rendering | Reviewed fixture-family text raster bucket; keep separate from true pattern/transparency rows. | #560 |
| `render-form-widget-bbox-clipping-equivalence-match` | 1 | forms and widget appearance BBox clipping | Reviewed fixture-scoped form/widget clipping bucket; keep separate from broad text placement/raster rows. | #558 |
| `render-form-widget-raster-equivalence-match` | 2 | forms and widget appearance rendering | Real form/widget renderer gap; reduce fixture by fixture. | #558 |
| `render-glyph-raster-equivalence-match` | 5 | fonts, glyph rasterization, and backend antialiasing | Reviewed glyph raster bucket; keep separate from rows with measurable glyph geometry drift. | #561 |
| `render-glyph-layout-equivalence-match` | 2 | fonts, glyph layout, and fallback rendering | Real glyph/font render fidelity gap; reduce fixture by fixture. | #561 |
...
| `render-java-optional-jpx-reader-missing-match` | 3 | optional Java JPEG 2000 provider | Accepted optional-runtime difference for the fixture-scoped JPX policy. | No default CI provider; see #542. |
...
| `render-foreground-shape-equivalence-match` | geometry/clipping | 5 |
...
| `render-form-widget-bbox-clipping-equivalence-match` | forms | 1 |
| `render-form-widget-raster-equivalence-match` | forms | 2 |
| `render-global-resource-text-raster-equivalence-match` | fonts/glyphs | 4 |
| `render-glyph-layout-equivalence-match` | fonts/glyphs | 2 |
| `render-glyph-raster-equivalence-match` | fonts/glyphs | 5 |
| `render-image-mask-shape-equivalence-match` | sampled images/masks | 2 |
| `render-java-optional-jpx-reader-missing-match` | optional runtime provider | 3 |
| `render-jbig2-decoder-raster-equivalence-match` | sampled images/masks | 1 |
| `render-lossy-jpeg-decoder-equivalence-match` | sampled images/masks | 2 |
| `render-pattern-transparency-raster-equivalence-match` | PageDrawer patterns/transparency | 1 |
| `render-rotation-overlay-shape-equivalence-match` | geometry/clipping | 9 |
...
| `PDFBOX-2725-878725.pdf` | backend antialiasing | 1.46218 | 5.59173 | 0.0838615 | 0.00229418 | `java/PDFBOX-2725-878725-java-p1.png` / `dotnet/PDFBOX-2725-878725-dotnet-p1.png` |
| `PDFBOX-2984-rotations.pdf` | geometry/clipping | 0.870905 | 10.8669 | 0.0133638 | 0.00917108 | `java/PDFBOX-2984-rotations-java-p1.png` / `dotnet/PDFBOX-2984-rotations-dotnet-p1.png` |
| `PDFBOX-3042-003177-p2.pdf` | backend antialiasing | 1.14363 | 5.34115 | 0.0638206 | 0.00176222 | `java/PDFBOX-3042-003177-p2-java-p1.png` / `dotnet/PDFBOX-3042-003177-p2-dotnet-p1.png` |
| ... | Additional rows omitted from Markdown; see JSON report. | | | | | |

### `render-rotation-overlay-shape-equivalence-match`

Owner/root cause: Rotation and overlay fixtures preserve page geometry and foreground shape while Java2D vs .NET line rasterization differs.
...
| `Overlayed-with-rot270.pdf` | geometry/clipping | 7.74852 | 21.2072 | 0.241599 | 0.0926091 | `java/Overlayed-with-rot270-java-p1.png` / `dotnet/Overlayed-with-rot270-dotnet-p1.png` |
| `Overlayed-with-rot90.pdf` | geometry/clipping | 7.74156 | 21.1876 | 0.241516 | 0.0925513 | `java/Overlayed-with-rot90-java-p1.png` / `dotnet/Overlayed-with-rot90-dotnet-p1.png` |
| `rot0.pdf` | geometry/clipping | 6.19987 | 26.9538 | 0.09135 | 0.0728 | `java/rot0-java-p1.png` / `dotnet/rot0-dotnet-p1.png` |
| `rot180.pdf` | geometry/clipping | 6.25162 | 27.0114 | 0.0932 | 0.07325 | `java/rot180-java-p1.png` / `dotnet/rot180-dotnet-p1.png` |
| `rot270.pdf` | geometry/clipping | 6.22478 | 26.9939 | 0.09255 | 0.0728 | `java/rot270-java-p1.png` / `dotnet/rot270-dotnet-p1.png` |
| ... | Additional rows omitted from Markdown; see JSON report. | | | | | |
...
Owner/root cause: JBIG2 decoded foreground shape is preserved while decoder/backend raster pixels differ.
...
Owner/root cause: GlobalResource merge fixtures preserve text content and foreground shape while text raster density differs.

| File | Source area | Mean | RMS | Moderate | Large | Artifacts |
|---|---|---:|---:|---:|---:|---|
| `PDFBox.GlobalResourceMergeTest.Doc01.decoded.pdf` | fonts/glyphs | 4.38203 | 13.1448 | 0.202084 | 0.0364117 | `java/PDFBox.GlobalResourceMergeTest.Doc01.decoded-java-p1.png` / `dotnet/PDFBox.GlobalResourceMergeTest.Doc01.decoded-dotnet-p1.png` |
| `PDFBox.GlobalResourceMergeTest.Doc01.pdf` | fonts/glyphs | 4.38203 | 13.1448 | 0.202084 | 0.0364117 | `java/PDFBox.GlobalResourceMergeTest.Doc01-java-p1.png` / `dotnet/PDFBox.GlobalResourceMergeTest.Doc01-dotnet-p1.png` |
| `PDFBox.GlobalResourceMergeTest.Doc02.decoded.pdf` | fonts/glyphs | 4.38203 | 13.1448 | 0.202084 | 0.0364117 | `java/PDFBox.GlobalResourceMergeTest.Doc02.decoded-java-p1.png` / `dotnet/PDFBox.GlobalResourceMergeTest.Doc02.decoded-dotnet-p1.png` |
| `PDFBox.GlobalResourceMergeTest.Doc02.pdf` | fonts/glyphs | 4.38203 | 13.1448 | 0.202084 | 0.0364117 | `java/PDFBox.GlobalResourceMergeTest.Doc02-java-p1.png` / `dotnet/PDFBox.GlobalResourceMergeTest.Doc02-dotnet-p1.png` |

### `render-form-widget-bbox-clipping-equivalence-match`

Owner/root cause: Widget appearance BBox-edge strokes and clipping preserve semantics while right/bottom edge pixels differ.
...
Owner/root cause: Glyph probe rows match identity and exact geometry; only Java2D vs .NET raster pixels differ.
...
|---|---|---:|---:|---:|---:|---:|---:|
| `AlignmentTests.pdf` | `render-glyph-raster-equivalence-match` | 1316 | 0 | 0 | 0 | 0 | 0 |
| `PDFBOX-3038-001033-p2.pdf` | `render-glyph-raster-equivalence-match` | 1747 | 0 | 0 | 0 | 0 | 0 |
| `PDFBOX-3062-002207-p1.pdf` | `render-glyph-raster-equivalence-match` | 1329 | 0 | 0 | 0 | 0 | 0 |
| `PDFBOX-3656-SF1199AEG (Complete).pdf` | `render-glyph-raster-equivalence-match` | 3089 | 0 | 0 | 0 | 0 | 0 |
| `arxiv-sample.pdf` | `render-glyph-raster-equivalence-match` | 2464 | 0 | 0 | 0 | 0 | 0 |
| `ControlCharacters.pdf` | `render-glyph-layout-equivalence-match` | 343 | 0 | 0.009 | 0 | 0.001 | 0 |
| `PDFBOX-5784.pdf` | `render-glyph-layout-equivalence-match` | 220 | 0 | 0 | 0 | 0.001 | 0 |
