...
public sealed class WasmBrowserSmokeTest
{
    ...
    private static string FindRepositoryRoot()
    {
        ...
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "PdfBoxNet.slnx")))
        {
            directory = directory.Parent;
        }

        return directory?.FullName
            ?? throw new DirectoryNotFoundException("Could not locate the pdfbox-net repository root.");
    }
}
