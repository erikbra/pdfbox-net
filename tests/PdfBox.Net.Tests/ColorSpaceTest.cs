...
public class ColorSpaceTest
{
    ...
    [Fact]
    public void PDICCBased_AdobeRgbProfile_ConvertsToSrgbWithSingleBatchedLookup()
    {
        PDICCBased colorSpace = CreateIccColorSpace(ColorProfiles.AdobeRGB1998.ToByteArray(), 3);
        float[] source = [0.8f, 0.2f, 0.1f];

        float[] first = colorSpace.ToRGB(source);
        for (int index = 0; index < 200; index++)
        {
            colorSpace.ToRGB([index / 200f, 0.35f, 0.65f]);
        }

        Assert.Equal(3, first.Length);
        Assert.Contains(first.Zip(source), pair => Math.Abs(pair.First - pair.Second) > 0.02f);
        Assert.Equal(1, colorSpace.GetColorTransformOperationCount());
    }

    [Fact]
    public void PDICCBased_CmykProfile_UsesProfileInsteadOfArithmeticFallback()
    {
        PDICCBased colorSpace = CreateIccColorSpace(ColorProfiles.USWebCoatedSWOP.ToByteArray(), 4);
        float[] source = [0f, 1f, 1f, 0f];

        float[] managed = colorSpace.ToRGB(source);
        float[] fallback = PDDeviceCMYK.Instance.ToRGB(source);

        Assert.Equal(3, managed.Length);
        Assert.Contains(managed.Zip(fallback), pair => Math.Abs(pair.First - pair.Second) > 0.02f);
        Assert.Equal(1, colorSpace.GetColorTransformOperationCount());
    }

    [Fact]
    public void PDICCBased_GrayProfile_IsReadAndConverted()
    {
        // Generated by Little CMS linkicc from its built-in *Gray22 and *Lab profiles.
        byte[] profile = File.ReadAllBytes(Path.Combine("Fixtures", "Images", "lcms-gray22.icc"));
        PDICCBased colorSpace = CreateIccColorSpace(profile, 1);

        float[] rgb = colorSpace.ToRGB([0.42f]);

        Assert.Equal(3, rgb.Length);
        Assert.All(rgb, component => Assert.InRange(component, 0f, 1f));
        Assert.Equal(1, colorSpace.GetColorTransformOperationCount());
    }

    [Fact]
    public void PDICCBased_MalformedProfile_PreservesAlternateFallback()
    {
        PDICCBased colorSpace = CreateIccColorSpace([0, 1, 2, 3, 4, 5], 3);

        float[] rgb = colorSpace.ToRGB([0.1f, 0.2f, 0.3f]);

        Assert.Equal([0.1f, 0.2f, 0.3f], rgb);
        Assert.Equal(0, colorSpace.GetColorTransformOperationCount());
    }
    ...
    public void ColorManagementContext_SelectsFirstUsableOutputIntentAndMatchesDeviceComponents()
    {
        ...
        using MemoryStream unknownProfile = new(ColorProfiles.USWebCoatedSWOP.ToByteArray());
        using MemoryStream cmykProfile = new(ColorProfiles.CoatedFOGRA39.ToByteArray());
        PDOutputIntent malformed = new(document, malformedProfile);
        PDOutputIntent unknown = new(document, unknownProfile);
        ((COSDictionary)unknown.GetCOSObject()).SetName(COSName.GetPDFName("S"), "Custom_OutputIntent");
        PDOutputIntent cmyk = new(document, cmykProfile);

        PDColorManagementContext? context = PDColorManagementContext.Create(
            [malformed, unknown, cmyk],
            PdfRenderingIntent.PERCEPTUAL);

        Assert.NotNull(context);
        Assert.Same(cmyk, context.OutputIntent);
    ...
    private static PDICCBased CreateIccColorSpace(byte[] profile, int components)
    {
        ...
        profileStream.SetItem(COSName.GetPDFName("Alternate"), components switch
        {
            1 => COSName.GetPDFName("DeviceGray"),
            4 => COSName.GetPDFName("DeviceCMYK"),
            _ => COSName.GetPDFName("DeviceRGB"),
        });
        using (Stream output = profileStream.CreateOutputStream())
        {
        ...
        array.Add(COSName.GetPDFName("ICCBased"));
        array.Add(profileStream);
        return Assert.IsType<PDICCBased>(PDColorSpaceFactory.Create(array, new PDResources()));
    }
}
