...
public class SkiaGlyphLayoutProcessorTest
{
    ...
    [Fact]
    public void ComputeGlyphs_RejectsMissingGlyphs()
    {
        using PDDocument document = new();
        using SkiaGlyphLayoutProcessor processor = new();
        using FileStream input = File.OpenRead(FontPath("LiberationSans-Regular.ttf"));
        ...
        ArgumentException exception = Assert.Throws<ArgumentException>(
            () => processor.ComputeGlyphs(font, "\U0010FFFF"));

        Assert.Contains("U+10FFFF", exception.Message);
    }

    [Fact]
    public void ComputeGlyphs_RemainsUsableAfterSourceStreamIsDisposed()
    {
        using PDDocument document = new();
        using SkiaGlyphLayoutProcessor processor = new();
        PDType0Font font;
        using (FileStream input = File.OpenRead(FontPath("LiberationSans-Regular.ttf")))
        {
            font = processor.LoadFont(document, input);
        }
        ...
        SkiaGlyphLayoutProcessor.ShapedGlyph[] glyphs = processor.ComputeGlyphs(font, "AV office");
