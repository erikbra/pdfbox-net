...
EXCLUDED_MODULES = {"pdfbox-layout-fop"}
...
def java_occurrences(source: str) -> tuple[list[Occurrence], list[Occurrence]]:
    ...
    declaration_pattern = re.compile(
        r"\b(?:private|protected|public)\s+static\s+(?:final\s+)?Logger\s+"
    ...
    guard_pattern = re.compile(rf"\b{logger_expression}\.(isDebugEnabled|isWarnEnabled)\s*\(")
...
def java_parameter_occurrences(source: str) -> list[Occurrence]:
    ...
    for match in re.finditer(r"\bLogger\s+logger\b", source):
...
def git_source_paths(upstream_root: Path, upstream_ref: str) -> list[str]:
    ...
    for target in targets:
        ...
        for match in call_pattern.finditer(source):
            usages.append(Occurrence(line_number(source, match.start()), target,
                                     ...
                                     one_line(balanced_call(source, match))))
        guard_pattern = re.compile(r"\bLOG\.IsEnabled\s*\(\s*LogLevel\.(Debug|Warning)\s*\)")
        for match in guard_pattern.finditer(source):
            usages.append(Occurrence(line_number(source, match.start()), target,
...
def main() -> int:
    ...
    source_paths = git_source_paths(upstream_root, upstream_ref)
    for source_path in source_paths:
        ...
        for declaration in declarations:
            match = re.search(r"\bLogger\s+([A-Za-z_$][A-Za-z0-9_$]*)\b", declaration.text)
            if match:
        ...
        guard_cs = [row for row in cs_usages if row.level in ("debug", "warning") and
